...
'__mac_block'([Exps]) ->
    transform(?cast_paren([?cast_atom(list),?cast_block(Exps)])).

%% Translate {e0 e1 ...} to a tuple if not handled by some other macro
'__mac_brace'([Exps]) ->
    transform(?cast_paren([?cast_atom(tuple),?cast_block(Exps)])).
...
%%      lists:map(fun (?ast_paren([F,A])) ->
%% 		       ?erl_atom(Name)=transform(F),
%% 		       ?erl_integer(Arity)=transform(A),
%% 		       {Name,Arity}
%% 	       end,Fs)}.
...
%% unary_op(Op,Arg) ->
%%     transform([mk_atom(op),mk_atom(Op),Arg]).

%% binary_op(Op,Arg1,Arg2) ->
%%     transform([mk_atom(op),mk_atom(Op),Arg1,Arg2]).

%% nested_binary_op(Op,Exp) ->
%%     transform(nested_binary_op_rec(Op,Exp)).

%% nested_binary_op_rec(Op,Exp) ->
%%     case Exp of
...
%% %% rem
%% '__mac_rem'([Arg1,Arg2]) ->
%%     binary_op('rem',Arg1,Arg2).

%% %% and
%% '__mac_and'(E) ->
%%     nested_binary_op(mk_atom('and'),E).

%% %% or
%% '__mac_or'(E) ->
%%     nested_binary_op(mk_atom('or'),E).
...
%% %% andalso
%% '__mac_andalso'(E) ->
%%     nested_binary_op(mk_atom('andalso'),E).

%% %% orelse
%% '__mac_orelse'(E) ->
%%     nested_binary_op(mk_atom('orelse'),E).
...
'__mac_call'([?ast_brace([Mod,Fn])|Body]) ->
    {call,lineno(),{remote,lineno(),transform(Mod),transform(Fn)},transform_each(Body)};
...
%% 	 compile1("(tuple: 1 2 3)")),
%% %% {tuple,1,[{integer,1,1},{integer,1,2},{integer,1,3}]}.
%% ?_assert(compile1("(tuple: 1 2 3)") ==
%% 	 compile1("{1 2 3}")),
%% %% {tuple,1,[{integer,1,1},{integer,1,2},{integer,1,3}]}.
%% ?_assert(erl_parse_e("[].") == 
%% 	 compile1("[]")),
...
%% 	 compile1("(if: [t11 t12] e1 {[t211 t212] [t221 t222]} e2 t3 e3)")),
%% %% {'if',1,
...
%% 	 compile1("(case E: (p1: when [t1 t2]: e1) (p2: when t21 [t221 t222]: e2) (t3: e31 e32))")),
%% %% {'case',1,
%% %% 	{var,1,'E'},
%% %% 	[{clause,1,
%% %% 		 [{atom,1,p1}],
...
%% 	 compile1("(try: e1 :catch (c1 ex1: h1) (ex2: when [(foo ex2) t1]: h2) (c3 ex3: when t3: h3))")),
%% %% {'try',1,
%% %%        [{atom,1,e1}],
%% %%        [],
%% %%        [{clause,1,
...
%% 	 compile1("(try: E :of (p1: e2) :after a1 a2)")),
%% %% {'try',1,
%% %%        [{var,1,'E'}],
%% %%        [{clause,1,[{atom,1,p1}],[],[{atom,1,e2}]}],
%% %%        [],
...
%% 	 compile1("(try: E :catch (ex1: h1) :after a1 a2)")),
%% %% {'try',1,
%% %%        [{var,1,'E'}],
%% %%        [],
%% %%        [{clause,1,
...
%% 	 compile1("(try: E :of (p1: when t1: p2) :catch (ex1: h1) :after a1 a2)")),
%% %% {'try',1,
%% %%        [{var,1,'E'}],
